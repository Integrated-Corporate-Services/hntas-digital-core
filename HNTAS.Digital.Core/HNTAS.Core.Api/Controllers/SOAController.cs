...
namespace HNTAS.Core.Api.Controllers
{
    ...
    public class SOAController : ControllerBase
    {
        ...
        {
            _logger.LogInformation("Retrieving SOA project for Heat Network ID: {HnId}", hnId.ToSafeLog());

            var project = await _soaService.GetByHeatNetworkIdAsync(hnId);

            if (project is null)
            {
                _logger.LogWarning("SOA project not found for Heat Network ID: {HnId}", hnId.ToSafeLog());
                return NotFound();
            }

            _logger.LogInformation("SOA project retrieved successfully for Heat Network ID: {HnId}", hnId.ToSafeLog());
            return Ok(project);
        }
        ...
        public async Task<IActionResult> CreateProject([FromQuery] string hnId, [FromQuery] string createdBy)
        {
            ...
            _logger.LogInformation("Initiating SOA creation for Heat Network ID: {HnId} by user: {CreatedBy}", hnId.ToSafeLog(), createdBy.ToSafeLog());

            var soa = await _soaService.CreateAsync(hnId, createdBy);

            if (soa is null)
            {
                _logger.LogWarning("SOA creation failed for Heat Network ID: {HnId}", hnId.ToSafeLog());
                return BadRequest("Unable to create SOA data.");
            }

            _logger.LogInformation("SOA data successfully created for Heat Network ID: {HnId}", hnId.ToSafeLog());
            return Ok(soa);
        }
        ...
        {
            _logger.LogInformation("Updating connection types for Heat Network ID: {HnId} by {UpdatedBy}", request.HnId.ToSafeLog(), request.UpdatedBy.ToSafeLog());
            ...
            if (project == null)
            {
                _logger.LogWarning("SOA project not found for connection update: {HnId}", request.HnId.ToSafeLog());
                return NotFound();
            }

            await _soaService.UpdateConnectionTypesAsync(request.HnId, request.UpdatedBy, request.ConnectionTypes);
            _logger.LogInformation("Connection types updated for Heat Network ID: {HnId}", request.HnId.ToSafeLog());
        ...
        {
            _logger.LogInformation("Updating network type for Heat Network ID: {hnId} by {UpdatedBy}", hnId.ToSafeLog(), updatedBy.ToSafeLog());
            ...
            if (project == null)
            {
                _logger.LogWarning("SOA project not found for network type update: {hnId}", hnId.ToSafeLog());
                return NotFound();
            }

            await _soaService.UpdateNetworkTypeAsync(hnId, updatedBy, networkTypeSelection);
            _logger.LogInformation("Network type updated for Heat Network ID: {hnId}", hnId.ToSafeLog());
        ...
        public async Task<IActionResult> SaveElementLocations([FromBody] UpdateElementLocationsRequest request)
        {
            ...
            _logger.LogInformation("Saving locations for element type: {ElementType} in HN ID: {HnId} by {UpdatedBy}. Location count: {LocationCount}",
                request.ElementType, request.HnId.ToSafeLog(), request.UpdatedBy.ToSafeLog(), request.Locations?.Count ?? 0);

            var project = await _soaService.GetByHeatNetworkIdAsync(request.HnId);
            if (project == null)
            {
                _logger.LogWarning("SOA project not found for location save: {HnId}", request.HnId.ToSafeLog());
                return NotFound();
            }
            ...
            try
            {
                ...
                _logger.LogInformation("Locations updated successfully for element type: {ElementType} in HN ID: {HnId} by {UpdatedBy}",
                    request.ElementType, request.HnId.ToSafeLog(), request.UpdatedBy.ToSafeLog());
                return Ok();
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> SaveElementDocuments([FromBody] UpdateElementDocumentsRequest request)
        {
            ...
            _logger.LogInformation("Saving documents for element type: {ElementType} in HN ID: {HnId} by {UpdatedBy}. Document count: {DocumentCount}",
                request.ElementType, request.HnId.ToSafeLog(), request.UpdatedBy.ToSafeLog(), request.Documents?.Count ?? 0);

            var project = await _soaService.GetByHeatNetworkIdAsync(request.HnId);
            if (project == null)
            {
                _logger.LogWarning("SOA project not found for document save: {HnId}", request.HnId.ToSafeLog());
                return NotFound();
            }
        ...
        public async Task<IActionResult> SaveDocument([FromBody] UpdateDocumentRequest request)
        {
            ...
            _logger.LogInformation("Saving {DocumentType} document for HN ID: {HnId}, Phase: {Phase}, Stage: {Stage}, UploadedBy: {UploadedBy}",
                request.DocumentType, request.HnId.ToSafeLog(), request.Phase, request.Stage, request.UploadedBy.ToSafeLog());

            var project = await _soaService.GetByHeatNetworkIdAsync(request.HnId);
            if (project == null)
            {
                _logger.LogWarning("SOA not found for {DocumentType} document save: {HnId}", request.DocumentType, request.HnId.ToSafeLog());
                return NotFound();
            }
        ...
        public async Task<IActionResult> DeleteSoaProject(string hnId)
        {
            ...
            _logger.LogInformation("Attempting to delete SOA project for HN ID: {HnId}", hnId.ToSafeLog());

            var project = await _soaService.GetByHeatNetworkIdAsync(hnId);
            if (project == null)
            {
                _logger.LogWarning("SOA project not found for deletion: {HnId}", hnId.ToSafeLog());
                return NotFound();
            }
            ...
            try
            {
                ...
                _logger.LogInformation("SOA project deleted successfully for HN ID: {HnId}", hnId.ToSafeLog());
                return NoContent();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to delete SOA project for HN ID: {HnId}", hnId.ToSafeLog());
                throw;
            }
        }
            ...
            [FromQuery][Required] string assessmentResult)
        {
            if (!ModelState.IsValid)
            {
                _logger.LogWarning("Invalid assessor assessment email request: hnName={HnName}, hnId={HnId}, result={Result}", hnName.ToSafeLog(), hnId.ToSafeLog(), assessmentResult.ToSafeLog());
                return BadRequest(ModelState);
            }
            ...
            try
            {
                ...
                {
                    _logger.LogWarning("No assessors found for HN ID: {HnId}", hnId.ToSafeLog());
                    return NotFound();
                }
            ...
            {
                _logger.LogError(ex, "Error sending assessment result email for HN ID: {HnId}", hnId.ToSafeLog());
            ...
            [FromQuery][Required] string hnId)
        {
            if (!ModelState.IsValid)
            {
                _logger.LogWarning("Invalid certification complete email request: hnName={HnName}, hnId={HnId}", hnName.ToSafeLog(), hnId.ToSafeLog());
                return BadRequest(ModelState);
            }
            ...
            try
            {
                ...
                foreach (var email in emailRecipients.Distinct())
                {
                    ...
                    _logger.LogInformation("Certification complete email sent to {EmailAddress} for HN ID: {HnId}", email.ToSafeLog(), hnId.ToSafeLog());
                }
            ...
            {
                _logger.LogError(ex, "Error sending certification complete email to {hnName}", hnName.ToSafeLog());
                return StatusCode(StatusCodes.Status500InternalServerError);
            }
        }
