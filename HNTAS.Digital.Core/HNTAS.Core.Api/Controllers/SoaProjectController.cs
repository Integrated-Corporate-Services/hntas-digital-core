...
namespace HNTAS.Core.Api.Controllers
{
    ...
    public class SoaProjectController : ControllerBase
    {
        ...
        {
            _logger.LogInformation("Retrieving SOA project..");

            var project = await _soaProjectService.GetByIdAsync(projectId);

            if (project == null)
            {
                _logger.LogWarning("SOA project not found.");
                return NotFound();
            }

            _logger.LogInformation("SOA project retrieved successfully.");
            return Ok(project);
        }
        ...
        {
            _logger.LogInformation("Retrieving SOA project for the provided Heat Network ID.");

            var project = await _soaProjectService.GetByHeatNetworkIdAsync(hnId);

            if (project == null)
            {
                _logger.LogWarning("SOA project not found for the provided Heat Network ID.");
                return NotFound();
            }

            _logger.LogInformation("SOA project retrieved successfully for the provided Heat Network ID.");
            return Ok(project);
        }
        ...
        {
            _logger.LogInformation("Creating new SOA project for the provided Heat Network ID by {UpdatedBy}", createdBy);
            ...
            _logger.LogInformation("SOA project created successfully.");
        ...
        {
            _logger.LogInformation("Updating connection types for the provided Heat Network ID by {UpdatedBy}", request.UpdatedBy);
            ...
            if (project == null)
            {
                _logger.LogWarning("SOA project not found for connection update.");
                return NotFound();
            }

            await _soaProjectService.UpdateConnectionTypesAsync(request.HnId, request.UpdatedBy, request.ConnectionTypes);
            _logger.LogInformation("Connection types updated for the provided Heat Network ID.");
        ...
        {
            _logger.LogInformation("Updating network type for the provided Heat Network ID by {UpdatedBy}",updatedBy);
            ...
            if (project == null)
            {
                _logger.LogWarning("SOA project not found for network type update.");
                return NotFound();
            }

            await _soaProjectService.UpdateNetworkTypeAsync(hnId, updatedBy, networkTypeSelection);
            _logger.LogInformation("Network type updated for the provided Heat Network ID.");
        ...
        public async Task<IActionResult> SaveElementLocations([FromBody] UpdateElementLocationsRequest request)
        {
            ...
            _logger.LogInformation("Saving locations for element type: {ElementType} in the provided HN ID by {UpdatedBy}. Location count: {LocationCount}",
            request.ElementType, request.UpdatedBy, request.Locations?.Count ?? 0);

            var project = await _soaProjectService.GetByHeatNetworkIdAsync(request.HnId);
            if (project == null)
            {
                _logger.LogWarning("SOA project not found for location save.");
                return NotFound();
            }
            ...
            try
            {
                ...
                _logger.LogInformation("Locations updated successfully for element type: {ElementType} in the provided HN ID by {UpdatedBy}",
                    request.ElementType, request.UpdatedBy);
                return Ok();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to update locations for element type: {ElementType} in the provided HN ID by {UpdatedBy}",
                    request.ElementType, request.UpdatedBy);
                throw;
            }
        }
